...
namespace :server do
  ...
  task :start do
    exec "thin --rackup config.ru --daemonize --log log/thin.log --pid tmp/pids/thin.pid --environment production --port 80 start && echo '> Marley started on http://localhost:80'"
  end
